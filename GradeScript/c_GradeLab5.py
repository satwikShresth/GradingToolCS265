...
class c_GradeLab5(c_AssignmentTrack):
    ...
    def m_gradedReport(self, student: c_Student):
        ...
        while True:
            ...
            if (selectedData == options[0]):
                self.m_regrade(
                    student, ["gcc", "-o", "Question2", student.ls_filenames[0]])
                self.m_CheckQuestion1(student)
            elif (selectedData == options[1]):
                self.m_regrade(
                    student, ["gcc", "-o", "Question2", student.ls_filenames[1]])
    ...
    def m_grade(self, name):
        ...
        student.s_feedback += self.funFact
        student.s_feedback += self.footer
    ...
    def m_testExecutableQuestion3(self, student: c_Student, testExe, outputFile):
        ...
        if len(output) >= 1:
            ...
            for key, value in dict.items():
                student.s_initialFeedback += f" {-1:<6}Failed Test String      -> {self.testStringsQ3[key][0]}\n"
                student.s_initialFeedback += f" {'':6}your ouput              -> {value}\n"
                student.s_initialFeedback += f" {'':6}desired output          -> {self.testStringsQ3[key][1]}\n"
                student.s_initialFeedback += f"-----------------------------------------------------------------------\n"
    ...
    def m_CheckQuestion1(self, student: c_Student):
        student.s_initialFeedback += f"-----------------------------------------------------------------------\n"
        student.ls_filenames[0] = self._CheckFile_(student, self.filesReq[0])
        student.s_initialFeedback += f"\nPart-1 Reverse the string\n"
        if (student.ls_filenames[0] != False):
            output = "Question1"
            notError = self.m_compileCfile(
                ["gcc", student.ls_filenames[0], "-o", output])
            if (notError == True):
                pointDeducted = self.m_testExecutableQuestion1(student, output)
                if (pointDeducted == 0):
                    student.s_initialFeedback += f"{'':6} All test strings passed\n"
                student.f_grade -= pointDeducted
            else:
                self.m_compilationError(
                    student, student.ls_filenames[0], notError, 5)

    def m_CheckQuestion2(self, student: c_Student):
        student.s_initialFeedback += f"\nPart-2 Reverse the string[*pointers]\n"
        student.s_initialFeedback += f"-----------------------------------------------------------------------\n"
        student.ls_filenames[1] = self._CheckFile_(student, self.filesReq[1])
        if (student.ls_filenames[1] != False):
            output = "Question2"
            notError = self.m_compileCfile(
                ["gcc", "-o", output, student.ls_filenames[1]])
            if (notError == True):
                subprocess.run(["less", f"{student.ls_filenames[1]}"])
                while (1):
                    grade = input("Grade[?/5]: ")
                    if grade.isdigit():
                        grade = int(grade)
                        if grade > 0 and grade < 6:
                            ...
                            student.f_grade += (grade-5)
                            if (grade-5) == 0:
                                student.s_initialFeedback += f"{' ':6} {feedback}\n"
                            else:
                                student.s_initialFeedback += f" {(grade-5):<6} {feedback}\n"
                            ...
                            student.s_initialFeedback += f" {-10:<6} Did not follow the instructions!\n"
                            student.f_grade -= 10
                            break
            else:
                self.m_compilationError(
                    student, student.ls_filenames[1], notError, 5)

    def m_CheckQuestion3(self, student: c_Student):
        student.s_initialFeedback += f"\nPart-3 Modifying split function accept 1 array, 2 int* and return int*\n"
        student.s_initialFeedback += f"-----------------------------------------------------------------------\n"
        student.ls_filenames[2] = self._CheckFile_(student, self.filesReq[2])
        if (student.ls_filenames[2] != False):
            output = "Question3.so"
            notError = self.m_compileCfile(
                ["gcc", "-shared", "-o", output, student.ls_filenames[2]])
            if (notError == True):
                student.s_initialFeedback += f"Testing QuickSort Function\n"
                self.m_testExecutableQuestion3(
                    student, self.f_gradeQuickSortExe, output)
                student.s_initialFeedback += f"Testing Split Function\n"
                self.m_testExecutableQuestion3(
                    student, self.f_gradeSplitExe, output)
            else:
                self.m_compilationError(
                    student, student.ls_filenames[2], notError, 10)
