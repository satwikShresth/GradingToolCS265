...
import os,subprocess,json,difflib,curses
...
class c_AssignmentTrack():
    ...
    def m_finalizeGrade(self,student:c_Student):
        student.initialFeedback  = student.initialFeedback .replace('?!?',str(student.grade))
...
class c_GradeLab5(c_AssignmentTrack):
    ...
    def m_gradedReport(self,student:c_Student):
        ...
        instructions =[ f"Student : {student.name}", f"Grade   : {student.grade}","Select an option:"]
        options = ["Regrade Question 1","Regrade Question 2","Regrade Question 3","Edit grade.file","Show grade.file","Continue"]
        while True:
            screen = curses.initscr()
            selectedData = self.uI.m_terminalUserInterface(screen,options,instructions)
            curses.endwin()
            if (selectedData == options[0]):
                self.m_regrade(student,["gcc","-o","Question2", student.filenames[0]])
                self.m_CheckQuestion1(student)
            elif (selectedData == options[1]):
                self.m_regrade(student,["gcc","-o","Question2", student.filenames[1]])
                self.m_CheckQuestion2(student)
            elif (selectedData == options[2]):
                self.m_regrade(student,["gcc","-shared","-o","Question3", student.filenames[2]])
    ...
    def m_grade(self,name):
        ...
        self.m_createFeedbackFile(student.feedbackFile,student.initialFeedback)
    ...
    def m_CheckQuestion1(self,student:c_Student):
        ...
        if( student.filenames[0] != False):
            output = "Question1"
            notError = self.m_compileCfile(["gcc", student.filenames[0], "-o", output])
            if(notError == True):
                ...
                self.m_compilationError(student,student.filenames[0],notError,5)
    ...
    def m_CheckQuestion2(self,student:c_Student):
        ...
        if( student.filenames[1] != False):
            output = "Question2"
            notError = self.m_compileCfile(["gcc", "-o", output,student.filenames[1]])
            if(notError == True):
                ...
                self.m_compilationError(student,student.filenames[1],notError,5)
    ...
    def m_CheckQuestion3(self,student:c_Student):
        ...
        if( student.filenames[2] != False):
            output = "Question3.so"
            notError = self.m_compileCfile(["gcc","-shared","-o",output,student.filenames[2]])
            if(notError == True):
                ...
                self.m_compilationError(student,student.filenames[2],notError,10)
